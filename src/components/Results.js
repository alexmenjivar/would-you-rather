        ...
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <div style={{ marginLeft: 20, width:'70%' }}>
              <h3>Results:</h3>
              <Card style={{ backgroundColor: userVote === 'One' ? 'aquamarine' : null}}>
                <Text strong>{`Would you rather ${optionOne.text}`}</Text>
                <Progress percent={Math.round((optionOne.votes/(optionOne.votes+optionTwo.votes))*100)}/>
                <div>{optionOne.votes} out of {optionOne.votes + optionTwo.votes} votes</div>
              </Card>
              <br />
              <Card style={{ backgroundColor: userVote === 'Two' ? 'aquamarine' : null}}>
                <Text strong>{`Would you rather ${optionTwo.text}`}</Text>
                <Progress percent={Math.round((optionTwo.votes/(optionOne.votes+optionTwo.votes))*100)} />
                <div>{optionTwo.votes} out of {optionOne.votes + optionTwo.votes} votes</div>
              </Card>
            </div>
          </div>
        </Card>
